...
$pets = $firebase->getDocuments("adoptionApplication");
...
if ($searchQuery) {
    $pets = array_filter($pets, function ($pet) use ($searchQuery) {
        ...
        return (
            (isset($pet['transactionNumber']) && stripos($pet['transactionNumber'], $searchQuery) !== false) ||
            ...
            (isset($pet['applicationStatus']) && stripos($pet['applicationStatus'], $searchQuery) !== false) ||
            (isset($pet['adoptionDate']) && stripos($pet['adoptionDate'], $searchQuery) !== false) ||
...
$pendingPets = array_filter($pets, function($pet) {
    return isset($pet['applicationStatus']) && $pet['applicationStatus'] === 'COMPLETED';
});
?>
...
<body>
  ...
  <div class="main-content">
    <div class="container-fluid mt-5 pt-3">
      <h1>Completed Adoption Application</h1>
      <p>Below is the list of Completed Adoption Application currently registered in the system:</p>
      ...
      <div class="input-group" style="width: 50%; margin: 0 auto;">
          ...
          <a href="applicationCompleted.php" class="btn btn-secondary">Reset</a>
      </div>
    </form>
        ...
        <table class="table table-hover mx-auto" style="width: 90%;">
          <thead class="table-success">
            ...
            <a href="?sortField=firstName&sortOrder=desc" class="sort-dropdown">↓</a>
              ...
              <th>Adoption Date
              <a href="?sortField=adoptionDate&sortOrder=asc" class="sort-dropdown">↑</a>
              <a href="?sortField=adoptionDate&sortOrder=desc" class="sort-dropdown">↓</a>
              </th>
              <th>Actions</th>
            </tr>
          </thead>
    ...
    <?php foreach ($pendingPets as $petid => $pet) : ?>
        <tr>
            <td><?= htmlspecialchars($pet['transactionNumber'] ?? 'N/A') ?></td>
            <td>
            ...
            <td><?= htmlspecialchars($pet['applicationStatus'] ?? 'N/A') ?></td>
                <td>
                <?php 
              ...
              ?>
                ...
                <?php 
                  // Display timestamp in formatted style
                  if (isset($pet['adoptionDate']) && $pet['adoptionDate'] !== 'N/A' && !empty($pet['timestamp'])) {
                      try {
                          $time = new DateTime($pet['timestamp']); 
                          echo $time->format('F j, Y / g:i A');
                      } catch (Exception $e) {
              ...
              ?>
                ...
                <td>
                    <a href="viewApplication/markAsViewedAppCompleted.php?petid=<?= urlencode($petid) ?>&collection=adoptionApplication" class="btn btn-primary btn-sm">View Application Form</a>
                </td>
              </tr>
            <?php endforeach; ?>
          <?php else : ?>
            <tr>
              <td colspan="5" class="text-center">No Completed applications found</td>  
          </tr>
          <?php endif; ?>
          </tbody>
        </table>
